...
describe('Custom Tests', function () {
  ...
  it('[assignment] ii. deposit 0.1 ETH in L1 -> withdraw 0.08 ETH in L2 -> assert balances', async () => {
    ...
    try {
      aliceReceiveUtxo = Utxo.decrypt(aliceKeypair, events[0].args.encryptedOutput, events[0].args.index)
    } catch (e) {
      // we try to decrypt another output here because it shuffles outputs before sending to blockchain
      aliceReceiveUtxo = Utxo.decrypt(aliceKeypair, events[1].args.encryptedOutput, events[1].args.index)
    }
  ...
  it('[assignment] iii. see assignment doc for details', async () => {
      ...
      const tornadoPoolBalance = await token.balanceOf(tornadoPool.address)
      console.log("TornadoPool balanace: ", tornadoPoolBalance.toString());
      ...
      // console.log("Bob send amount: ", bobSendAmount.toString())
      expect(bobReceiveUtxo.amount).to.be.equal(bobSendAmount)

      // Bob withdraws a part of his funds from the shielded pool
      const bobWithdrawAmount = bobSendAmount
      const bobEthAddress = '0xDeaD00000000000000000000000000000000BEEf'
      const bobChangeUtxo = new Utxo({ amount: bobSendAmount.sub(bobSendAmount), keypair: bobKeypair })
      await transaction({
        tornadoPool,
      ...
      console.log(aliceWithdrawAmount.toString())
